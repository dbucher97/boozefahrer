...
class Game {
  ...
  disconnected(id) {
    ...
    if (idx !== -1) {
      ...
      if (this.state.name === 'who') {
        const sidx =
            this.state.users.findIndex((name) => name === this.users[idx].name);
        if (sidx >= 0) {
          ...
          this.state.name === 'bus' &&
          this.state.busfahrer === this.users[idx].name) {
        this.state = idleState;
        this.advanceState();
      }
      this.updateUsers();
  ...
  advanceState() {
    ...
    switch (this.state.name) {
      case 'idle':
        if (getShape(this.settings.shape.name)
                    .getTotal(this.settings.shape.rows) +
          ...
          this.timeout = setTimeout(() => {
            ...
            this.print(`New game started with ${
                this.users.map(({name}) => name).join(', ')}.`);
            this.updateUsers();
            this.advanceState();
          }, 1000);
        } else {
        ...
        if (this.state.rowsPlayed === this.settings.shape.rows) {
          const cardsPlayed =
              this.users.filter(({disconnected}) => !disconnected)
                  .map(
                      (user) => this.settings.playerCards -
                          Object.values(this.state.cardsPlayed)
                              .filter(({by}) => by === user.name)
                              .length,
                  );
          const maxCards = Math.max(...cardsPlayed);
          const users = this.users.filter(({disconnected}) => !disconnected)
    ...
    if (this.state.name === 'idle' || this.state.name === 'who' ||
  ...
  advanceBus() {
    ...
    if (this.state.busstate === '') {
      cardsDealt = [];
      [...Array(this.settings.bussize).keys()].map(
          (idx) => cardsDealt.push({stack: idx, zIndex: 0}));
      previousState = this.state.previousState;
    } else if (this.state.busstate === 'pause_deal') {
      const zIndex = this.state.cardsDealt.reduce(
          (prev, curr) =>
              (curr.stack === this.state.currentStack ? prev + 1 : prev),
          0,
      );
      cardsDealt = [
        ...this.state.cardsDealt,
        {stack: this.state.currentStack, zIndex: zIndex}
      ];
      busstate = 'pause_next';
      ...
      const lower = this.stack[cardsDealt.findIndex(
          (card) => card.stack === currentStack && card.zIndex === zIndex - 1)];
      if ((this.state.action === 'higher' && higher(upper, lower)) ||
      ...
      if (currentStack >= this.settings.bussize ||
          this.stack.length === cardsDealt.length) {
        return true;
      }
    }
    this.state = {
  ...
  broadcastCardAudio(id) {
    this.users.filter((user) => user.id !== id)
        .map((user) => this.io.to(user.id).emit('card audio', 1));
  }
  ...
  toggleReady(id) {
    if (this.state.name !== 'who' && this.state.name !== 'bus') {
      ...
      if (this.users
              .reduce(({ready, disconnected}, user) => {
                return {
                  ready: (ready || disconnected) &
                      (user.ready || user.disconnected),
                  disconnected: false,
                };
              })
              .ready) {
  ...
  playCard(id, {idx, onIdx}) {
    ...
    if (this.state.name === 'give' && user) {
      ...
      const sameOnIdx =
          Object.keys(this.state.playedThisRow)
              .filter(
